...
class CTFTime_OAuth_Backend (BaseBackend):
    def authenticate (self, request):
        ...
        if oauth_api_resp.status_code != 200:
            print(f'CTFTime OAuth: call to API endpoint {OAUTH.ctftime.api_base_url} returned {oauth_api_resp.status_code} {oauth_api_resp.reason}')
            return None
        ...
        try:
            ...
            if team_emails.count() == 0:
                new_team_email = CTFTeam_ContactEmails (
                    team=existing_team,
                    email=oauth_user_information['email']
                )
                new_team_email.save()
            ...
            new_ctftime = CTFTime_Team (
                ctftime_id=oauth_user_information['team']['id'],
                team=new_team
            )
            new_team_email = CTFTeam_ContactEmails (
                team=new_team,
