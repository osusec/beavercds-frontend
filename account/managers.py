...
class CTFTeamManager(BaseUserManager):
    def _create (self, team_name, password=None, ctftime_bool=False, bracket=None):
        if not team_name:
    ...
    def create_user (self, team_name, password=None, ctftime_bool=False, bracket=None):
        new_team = self._create (team_name, password, ctftime_bool, bracket)
        new_team.save(using=self._db)
        return new_team


    def create_superuser (self, team_name, password=None, ctftime_bool=False, bracket=None):
        new_team  = self._create (team_name, password, ctftime_bool, bracket)
