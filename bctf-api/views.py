...
class UpdateBrackets(View):
    ...
    def post (self, request):
        ...
        result_state = []
        current_state = set(CTFTeam_Bracket.objects.values_list('bracket_name', flat=True))
        ...
        with transaction.atomic():
            for bracket_json in new_json_state:
                ...
                access_hash = None if open_bracket else make_password(bracket_json["password"])

                if bracket_name in current_state:
                    # Currently existing bracket
                    (
                        CTFTeam_Bracket.objects
                        .filter(bracket_name=bracket_name)
                        .update(
            ...
            for old_bracket in current_state:
                ...
                (
                    CTFTeam.objects
                    .filter(bracket__bracket_name=old_bracket)
                    .update(bracket=None)
                )
                # Delete the bracket
                (
                    CTFTeam_Bracket.objects
                    .filter(bracket_name=old_bracket)
                    .delete()
                )
        # end: with transaction.atomic()
